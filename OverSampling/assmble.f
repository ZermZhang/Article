...
c*********************************************
c	if (mx*nx .eq. 2048) then
c                nxstart=-511
c                nystart=-511
c        else if (mx*nx .eq. 1024) then	
c		nxstart=-255
c		nystart=-255
c	else if (mx*nx .eq. 512 ) then
c		 nxstart=-127
c                 nystart=-127
c        else if (mx*nx .eq. 256 ) then
c                 nxstart=-63
c                 nystart=-63
c        else if (mx*nx .eq. 64) then
c                 nxstart=-31
c                 nystart=-31
c	else 
         ...
         do 90 l=1,4
            ...
            do 80 i=1,nx
               do 70 j=1,ny
                  ...
                  stf(k,l)=stf(k,l)
     +                     +p(l,n1)*p(k,n1)*stf1(1,1)
     +                     +p(l,n1)*p(k,n2)*stf1(1,2)
     +                     +p(l,n1)*p(k,n3)*stf1(1,3)
     +                     +p(l,n1)*p(k,n4)*stf1(1,4)
     +                     +p(l,n2)*p(k,n1)*stf1(2,1)
     +                     +p(l,n2)*p(k,n2)*stf1(2,2)
     +                     +p(l,n2)*p(k,n3)*stf1(2,3)
     +                     +p(l,n2)*p(k,n4)*stf1(2,4)
     +                     +p(l,n3)*p(k,n1)*stf1(3,1)
     +                     +p(l,n3)*p(k,n2)*stf1(3,2)
     +                     +p(l,n3)*p(k,n3)*stf1(3,3)
     +                     +p(l,n3)*p(k,n4)*stf1(3,4)
     +                     +p(l,n4)*p(k,n1)*stf1(4,1)
     +                     +p(l,n4)*p(k,n2)*stf1(4,2)
     +                     +p(l,n4)*p(k,n3)*stf1(4,3)
     +                     +p(l,n4)*p(k,n4)*stf1(4,4)
70               continue
80            continue
90           continue
100         continue
...
C     -----------------------------------------------
      ...
      real*8 basis_x,basis_y,coor_gauss,d,x0,y0,permc
...
c      y=0.0d0
c      do node=1,3
c        x=x+coor(node,1,i,j)
c        y=y+coor(node,2,i,j)
c      end do
c      x=x/4.0d0
c      y=y/4.0d0
c      permc=coef(x,y)
...
c               stf(k,l)=area*(bk1*bl1+bk2*bl2)*permc
            end do
         end do
      end do
