...
c
c      open(1,file='../dat/result/linear_1024.dat',form='formatted')
      open(1,file='../dat/result/exact_2048.dat',form='formatted')
      read(1,*)nxfine,nyfine,nm,(u_ex(k),k=1,nxfine*nyfine)   
      close(1)
      nxfine=nxfine+1
      nyfine=nyfine+1
...
cccccccccccccccccccccccccc
 	...
 	print*, 'coarse mesh size  ', nxglobal
 	print*, 'fine mesh size ', nxlocal
...
C     --------------------------------------------------
	...
	real*8 permc
      ...
      do i=1,4
       ...
       euxtmp=euxtmp+uec(i)*uec(j)*A(i,j)   !*abs(coef(x,y))
      end do
      end do
      end
C     -------------------------------------------------
